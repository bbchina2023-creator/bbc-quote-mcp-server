...
const SERVER_VERSION = "1.0.0-dev016.40";
const MCP_ORIGIN = "https://bbc-quote-mcp-server.bbchina2023.workers.dev";
...
function requireOAuthConfiguration(env: Env): void {
  if (!String(env.GITHUB_CLIENT_ID || "").trim()) {
    throw new Error("GITHUB_CLIENT_ID secret is not configured");
  }
...
async function exchangeGitHubCode(
  ...
  if (!payload.access_token) {
    throw new Error(
      payload.error_description || payload.error || "GitHub access token missing",
    );
  }
...
async function finishGitHubAuthorization(
  ...
  if (githubError) {
    return new Response(`GitHub authorization failed: ${githubError}`, {
      status: 403,
    });
  }
  ...
  try {
    ...
    const grantedScopes = oauthRequest.scope.filter((scope) =>
